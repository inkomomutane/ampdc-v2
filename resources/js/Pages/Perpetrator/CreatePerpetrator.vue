...
const addPerpetrator = ref(false);
const nameInput = ref();

const addPerpetratorTrigger = () => {
    addPerpetrator.value = true;
};

const closeCreatePerpetratorModal = () => {
    addPerpetrator.value = false;
    form.reset();
};
...
const createPerpetrator = () => {
    form.post(route("perpetrator.store"), {
        preserveScroll: true,
        onSuccess: () => closeCreatePerpetratorModal(),
...
<template>
    ...
    >
        ...
        <span class="mx-4">Adicionar perpetrador</span>
    </button>

    <Modal :show="addPerpetrator" @close="closeCreatePerpetratorModal">
        <div class="relative bg-white rounded shadow dark:bg-gray-700">
            <button
                class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white"
                type="button"
                @click="closeCreatePerpetratorModal"
            >
                <svg
                    aria-hidden="true"
                    class="w-5 h-5"
            ...
            <div class="px-6 py-6 lg:px-8">
                ...
                <form class="space-y-6" @submit.prevent="createPerpetrator">
